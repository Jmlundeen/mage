...
public class SeatView implements Serializable {
    ...
    public static SeatView fromProto(ViewProto.SeatView proto) {
        return new SeatView(proto);
    }

    // Private constructor for fromProto
    private SeatView(ViewProto.SeatView proto) {
        this.flagName = proto.getFlagName();
        this.playerId = proto.getPlayerId().isEmpty() ? null : UUID.fromString(proto.getPlayerId());
        this.playerName = proto.getPlayerName();
        this.playerType = proto.getPlayerType().isEmpty() ? null : PlayerType.valueOf(proto.getPlayerType());
