...
public final class AnuridScavenger extends CardImpl {
    ...
    public AnuridScavenger(UUID ownerId, CardSetInfo setInfo) {
        ...
        this.addAbility(new BeginningOfUpkeepTriggeredAbility(new SacrificeSourceUnlessPaysEffect(new AnuridScavengerCost())));
    }
        ...
        if (controller != null) {
            if (this.getTargets().choose(Outcome.Removal, controllerId, source.getSourceId(), source, game)) {
                for (UUID targetId: this.getTargets().get(0).getTargets()) {
                    ...
                    paid |= controller.moveCards(card, Zone.LIBRARY, source, game);
                }
            }
